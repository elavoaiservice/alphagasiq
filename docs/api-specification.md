...
| Method | Path | Notes |
|---|---|---|
| GET | `/admin/data-feeds` | requires `admin.data_feeds`. Every registered provider's config merged with its *live* `health_check()` result and its static dependency-map entry — connection status, `freshness_status` (`LIVE`/`CURRENT`/`DELAYED`/`STALE`/`FAILED`/`UNKNOWN`), priority, affected agents/business functions, dependency chain (spec §35). `data_quality_score` is the `DataQualityService`-computed average (0-100) from the provider's last successful ingestion, `null` only if it has never successfully ingested. `license_type`/`public_or_commercial`/`redistribution_allowed`/`ai_processing_allowed` (spec §31) are `null` for a provider that hasn't declared/verified its source's licensing terms — never defaulted to permissive |
...
| GET | `/alpha/market-bias` | **Ungated** (no permission required — a platform-wide computed indicator, not organization-scoped Alpha Intelligence content; see `docs/data-sources.md` §8). Query param: `market` (defaults to the platform's primary instrument). Returns a `MarketBiasResult` — deterministic label/score/driver breakdown, never an LLM's judgment call. Summarized on `/market/summary` as `market_bias`/`market_bias_score` |
