...
CATALOG: list[ConfigItem] = [
    ...
    ConfigItem("NOAA_API_TOKEN", "NOAA API token", "Data feeds", secret=True, placeholder="get free at ncei.noaa.gov/cdo-web/token",
               help="LIVE + free — weather / degree-days."),
    ConfigItem("SEC_EDGAR_CONTACT_EMAIL", "SEC EDGAR contact email", "Data feeds", placeholder="you@company.com",
               help="LIVE with just a contact email — SEC EDGAR filings."),
    ConfigItem("RSS_NEWS_FEEDS", "News RSS feeds", "Data feeds", placeholder="https://…/rss, https://…/rss",
               help="LIVE + free. Comma-separated RSS URLs. Set feeds, then turn off 'Use mock news'."),
    # ── Market data (paid / licensed — connectors are stubs) ──
    ConfigItem("CME_API_ID", "CME API ID", "Market data (paid)", placeholder="paid subscription"),
    ConfigItem("CME_API_SECRET", "CME API secret", "Market data (paid)", secret=True, placeholder="paid subscription"),
    ConfigItem("ICE_API_ID", "ICE API ID", "Market data (paid)", placeholder="paid subscription"),
    ConfigItem("ICE_API_SECRET", "ICE API secret", "Market data (paid)", secret=True, placeholder="paid subscription"),
    ConfigItem("NEWS_PROVIDER_API_KEY", "Licensed news API key", "Market data (paid)", secret=True, placeholder="optional",
               help="Licensed newswire — connector is a stub; use RSS feeds above for free live news."),
    ConfigItem("FERC_API_KEY", "FERC eLibrary key", "Market data (paid)", secret=True, placeholder="no public API — stub",
               help="FERC eLibrary is a document portal with no stable public JSON API; connector is a stub. Key stored for when it's built."),
    ConfigItem("PIPELINE_EBB_API_KEY", "Pipeline EBB key", "Market data (paid)", secret=True, placeholder="no common API — stub",
               help="Pipeline electronic bulletin boards have no common API; connector is a stub."),
    # ── Demo-mode toggles ──
    ConfigItem("USE_MOCK_MARKET_DATA", "Use mock market data", "Data feeds", kind="bool",
               help="ON = simulate Henry Hub prices (CME/ICE). No free live price feed exists — leave ON unless you've wired a paid feed."),
    ConfigItem("USE_MOCK_NEWS", "Use mock news", "Data feeds", kind="bool",
               help="ON = simulated news. Turn OFF once you've set real RSS feeds above."),
    # ── Enterprise storage (S3) ──
    ConfigItem("S3_ENDPOINT_URL", "S3 endpoint URL", "Enterprise storage", placeholder="optional (S3-compatible)"),
    ConfigItem("S3_BUCKET", "S3 bucket", "Enterprise storage", placeholder="alphagasiq-dev"),
    ConfigItem("S3_ACCESS_KEY", "S3 access key", "Enterprise storage", secret=True, placeholder="optional"),
    ConfigItem("S3_SECRET_KEY", "S3 secret key", "Enterprise storage", secret=True, placeholder="optional"),
    # ── Auth / OIDC ──
...
async def reload_runtime(state) -> dict:
    ...
    try:
        ...
        pass
    # Re-fetch market data with the now-current providers: seed() runs BEFORE this
    # overlay, so it used the pre-overlay (mock) ones. This makes a change to
    # USE_MOCK_MARKET_DATA (mock ↔ real Henry Hub / TTF) take effect on Reload/boot.
    try:
        await state.refresh_market_data()
    except Exception:
        pass
